...
def purge_attribute(attr_name):
    ...
    def _execute_purge_attribute(obj):
        if hasattr(obj, attr_name):
            getattr(obj, attr_name).clear()
    return _execute_purge_attribute


@shared_directive(dicts=())
def remove_attribute(attr_name, name):
    """Removes single element name from attribute container attr_name"""
...
@shared_directive(dicts=())
def set_attribute_value(attr_name, name, **kwargs):
    """For each of the kw arg=val pairs, set the name component within
    attribute attr_name with each of the pairs"""
    def _execute_set_attribute_value(obj):
        if hasattr(obj, attr_name):
            attr_obj = getattr(obj, attr_name)
            ...
            name_obj.update(kwargs)

            attr_obj.update({name: name_obj})

    return _execute_set_attribute_value


@shared_directive(dicts=())
def append_attribute_value(attr_name, name, **kwargs):
    """For each of the kw=val pairs, append their contents to
    component name in attribute attr_name"""
    def _execute_append_attribute_value(obj):
        if hasattr(obj, attr_name):
            attr_obj = getattr(obj, attr_name)
            ...
            for kw, val in kwargs.items():
                if kw in name_obj:
                    current_val = name_obj.get(kw)
                else:
                    current_val = []

                if (isinstance(current_val, Iterable) and
                    not isinstance(current_val, 'str')):
                    ...
                    update_dict.update({kw: current_val})

            name_obj.update(update_dict)
            attr_obj.update({name: name_obj})

    return _execute_append_attribute_value
